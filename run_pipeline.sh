#!/bin/bash
# Dual-Stream Deepfake Detection Pipeline Runner

set -e

echo "============================================"
echo "DUAL-STREAM DEEPFAKE DETECTION PIPELINE"
echo "============================================"
echo ""
...
if [ ! -f "config.yaml" ]; then
    echo "Error: config.yaml not found. Please run from project root."
    exit 1
fi
...
# Step 2: Training
python train.py --config config.yaml 2>&1 | tee training_output.log
...
if [ -f "checkpoints/best_model.pth" ]; then
    ...
    python evaluate.py --checkpoint checkpoints/latest.pth --split test
fi
